...
metadata = {
    ...
    "protocolName": "Claw over falcon tube",
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    pipette.default_speed = 50 # 50 mm/s
    
    # Define the sequence of deck positions we want to visit
    deck_positions = [3, 4, 6, 7, 9]

    # Move pipette and labware sequentially through positions 2 to 10
    for position_index in deck_positions:
        ...
        subprocess.run(["python3", "/var/lib/jupyter/notebooks/Claw/Claw_grab_heavy.py"]) #this works
        
        protocol.delay(seconds=1)

        # Move the plate to the next deck position
        protocol.move_labware(labware=plate, new_location=str(position_index))
        ...
        subprocess.run(["python3", "/var/lib/jupyter/notebooks/Claw/Claw_release_heavy.py"])  # Runs this code before it moves 
